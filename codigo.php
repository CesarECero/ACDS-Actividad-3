...
function calcularValorInventario() {
    ...
    foreach ($inventarioCache as $p) {
        $total += $p["precio"] * $p["stock"]; // se arreglo el cálculo del valor del inventario multiplicando el precio por el stock de cada producto
    }
...
while ($i < count($inventarioCache)) {
    ...
    $i++; // se arreglo el bucle infinito incrementando el contador
}
